    ...
    if config.useParallelChannels
        ...
        parfor ch = 1:nChannels
            bestRows{ch} = search_one_channel( ...
                dataset, channelNames{ch}, searchContext, config);
...
function tableOut = sort_correlation_table(tableIn, criterion)
    switch lower(criterion)
        case 'min'
            tableOut = sortrows( ...
                tableIn, 'SpearmanRho', 'ascend');

        case 'max'
            tableOut = sortrows( ...
                tableIn, 'SpearmanRho', 'descend');

        case 'maxabs'
            [~,order] = sort( ...
                abs(tableIn.SpearmanRho), 'descend');
